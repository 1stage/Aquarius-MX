...
; of each of the three channels in the AY sound chip, stored in registers held in
; the AYREGS data structure.

Animation:
        PUSH HL                     ; Save existing HL values on stack
        PUSH DE                     ; Save existing DE values on stack
        PUSH BC                     ; Save existing BC values on stack
        LD   HL,$3400+(40*20)+13    ; Set HL = starting screen address of bars (in COLRAM):
        ...
        POP  HL                     ; Restore the saved HL value from the stack
        RET                         ; Return

;--------------------------------------------
;   Show/draw vertical bar 0-15 characters
;--------------------------------------------
;  in: A = Current "filled" bar height (from channel amplitude/volume)
;      D = "filled" bar color, (FG * 16) + BG
;      E = "empty"  bar color, (FG * 16) + BG
;     HL = Screen address of part of bar being drawn
;     BC = Serves as the screen offset for the NEXT location for HL

show_bar:
        LD   B,15                   ; Set B to max height of bars
        LD   C,D                    ; Set C to the "filled" bar color
        INC  A                      ; Increment the bar height value
.bar11  DEC  A                      ; Decrement the bar height value
        JR   NZ,.bar12              ; If the bar height value is not zero, jump to the .bar12 label...
        LD   C,E                    ; ...otherwise, set C to the "empty" color
.bar12  LD   (HL),C                 ; Put the value of the "empty" color into the current bar screen location
        INC  HL                     ; Move the current screen location one to the right
        LD   (HL),C                 ; Put the value of the "empty" color into the current bar screen location
        INC  HL                     ; Move the current screen location one to the right
        LD   (HL),C                 ; Put the value of the "empty" color into the current bar screen location
        PUSH BC                     ; Save current BC offset to stack (we need it later)
        LD   BC,-42                 ; Set BC offset to the value -42 (up 1 row and left 2)
        ADD  HL,BC                  ; Set HL (current screen location) to decrement by the BC offset: 
                                    ;       up 1 row and left 2
        POP  BC                     ; Retrieve saved BC offset from stack
        DJNZ .bar11                 ; If the offset is not zero, jump to the .bar11 label...
        LD   BC,(15*40)+5           ; ...otherwise, set the BC offset to the location of the next bar:
                                    ;       down 15 and right 5
        ADD  HL,BC                  ; Update HL to location of next bar (previous location + offset from BC)
        RET                         ; Return
...
SelectWindow:
      ...
      db   MAGENTA*16+BLACK             ; border colors
...
BgdWindow:
      db   (1<<WA_BORDER)|(1<<WA_TITLE)|(1<<WA_CENTER) ; attributes
      db   BLACK*16+BLACK               ; text colors
      db   BLACK*16+BLACK               ; border colors
      db   0,1,40,25                    ; x,y,w,h
      dw   0                            ; no title
